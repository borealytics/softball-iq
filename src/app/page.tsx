    ...
    <main className="min-h-screen bg-gradient-to-b from-green-800 via-green-700 to-slate-800 text-white">
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="bg-white/5 rounded-3xl p-4 backdrop-blur-sm border border-white/10">
          ...
          <p className="text-center text-sm text-green-100 mt-2">
            {posSurvol
              ? posSurvol.role
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-6">
          {POSITIONS.map((p) => {
            const nb = situationsParPosition(p.id).length;
            return (
              <button
                key={p.id}
                ...
                className="group relative bg-white rounded-2xl p-4 text-left shadow-md hover:shadow-xl hover:-translate-y-0.5 transition-all"
              >
